...
export const DrawerFilter: React.FC<DrawerFilterProps> = ({
    ...
    const handleLocationChange = (event: SelectChangeEvent<string[]>) => {
        ...
        setSelectOpen(false); // Close the menu after selection
    };
    ...
    const handleDeleteCity = (event: any, city: any) => {
        event.stopPropagation();  // Prevent event from propagating to parent elements
        setLocalFilterState((prevState) => ({
            ...prevState,
            locations: prevState.locations.filter(c => c !== city)
        }));
    };
                                ...
                                <Box sx={{display: 'flex', flexWrap: 'wrap', gap: 0.5}}>
                                    ...
                                    {selected.map((city) => (
                                        <Chip
                                            key={city}
                                            label={city}
                                            onDelete={(event) => handleDeleteCity(event, city)}
                                            deleteIcon={
                                                <CancelIcon
